...
class OrderbookSync extends WebsocketClient {
  ...
  _newProduct(productID) {
    ...
    getProductOrderBook(productID, options) {
	return limiter.schedule(() => this._client.getProductOrderBook(productID, options))
    }
  ...
  loadOrderbook(productID) {
    ...
    const process = data => {
	  ...
	  if (err.message.includes("429")) {
	      ...
	      setTimeout(() => { this.loadOrderbook(productID), 5 * 1000 })
	  } else {
